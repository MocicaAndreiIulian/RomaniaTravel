                                           ...
                                           <td>{{$comment->id}}</td>
                                            <td>{{$comment->articleTitle}}</td>
                                              <td>{{$comment->userName}}</td>
                                                  <td>{{$comment->rating}} <i class="fa fa-star"></i></td>
                                                        <td>{{$comment->comment}}</td>

                                           <td>
                                           <div class="btn-group" role="group" aria-label="Basic example">
                                              @if($comment->approve === 0)
                                              <a href={{"comment-approve/".$comment->id}}><button style="margin-right: 5px"  type="submit" class="btn btn-success">Aprobă</button></a>
                                              @else
                                              <button style="margin-right: 5px" type="submit" class="btn btn-success" disabled>Aprobă</button></a>
                                              @endif
                                               <a href={{"comment-delete/".$comment->id}}><button style="margin-left: 5px" type="submit" class="btn btn-danger">Șterge</button></a></td>
                                               </div>
                                           </td>
